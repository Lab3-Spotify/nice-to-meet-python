...
count_dict = {}
for c in input_str:
    count_dict[c] = count_dict.setdefault(c, 0) + 1

sorted_key = sorted(list(count_dict.keys()),key=lambda k: count_dict[k], reverse=True) # 判斷dict中value的值，將根據count排序過後的key存在sorted_key裡
